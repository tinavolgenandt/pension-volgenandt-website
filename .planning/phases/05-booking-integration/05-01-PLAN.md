...
- If consented, dynamically inject jQuery (from CDN, e.g., `https://code.jquery.com/jquery-3.7.1.min.js`) and then `bookWidget.min.js` into the document head
...
Style the container with Tailwind classes: rounded-lg, p-4/p-6, bg-white border border-sage-200, shadow-sm. Add a heading like "Verfügbarkeit & Buchung" above the widget.
...
- Include a section heading like "Verfügbarkeit & Buchung"
...
- Add a more prominent CTA button at the bottom of each card (e.g., "Zimmer ansehen" or "Verfügbarkeit prüfen") styled with waldhonig-500 (the CTA color)
