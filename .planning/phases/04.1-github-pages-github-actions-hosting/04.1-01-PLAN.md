...
<objective>
Create the GitHub Actions deployment workflow, fix pnpm config for CI compatibility, and migrate the contact form from Netlify Forms to Formspree (placeholder ID).

Purpose: This plan produces all the code and configuration needed for GitHub Pages deployment at the standard github.io URL. The contact form is migrated to Formspree code with a placeholder ID (account creation deferred). After this plan, the only remaining step is setting GitHub Pages source to "GitHub Actions" in repo settings.

Output: GitHub Actions workflow YAML, cleaned package.json, updated contact form component, updated app config.
</objective>
...
<task type="auto">
  <name>Task 1: Fix pnpm config and create GitHub Actions workflow</name>
...
5. `pnpm nuxt build --preset github_pages` (NOT `pnpm generate` -- the preset creates `.nojekyll` and `404.html` automatically). Set env var `NUXT_APP_BASE_URL: /pension-volgenandt-website/` so all asset paths work under the repo subpath on github.io.
...
**Note:** No CNAME file -- the site will be hosted at the standard github.io URL (`tinavolgenandt.github.io/pension-volgenandt-website/`) for now. Custom domain migration is deferred.
  </action>
  <verify>
...
3. Verify workflow file exists: `cat .github/workflows/deploy.yml` -- should contain `github_pages`, `deploy-pages@v4`, `pnpm/action-setup@v4`, `NUXT_APP_BASE_URL`
4. Validate YAML syntax: visually confirm proper indentation
  </verify>
  <done>
...
- Workflow sets `NUXT_APP_BASE_URL: /pension-volgenandt-website/` for repo subpath hosting
...
2. Run `pnpm nuxt build --preset github_pages` -- build succeeds and `.output/public/` directory contains site files, `.nojekyll`, and `404.html`
3. Verify `.output/public/.nojekyll` exists (created by preset)
4. Verify no Netlify references remain: `grep -r "netlify" app/components/contact/` returns no matches
5. Verify no `ignoredBuiltDependencies` in package.json
</verification>

<success_criteria>
- GitHub Actions workflow at `.github/workflows/deploy.yml` with build + deploy pipeline
- Workflow sets baseURL for repo subpath hosting on github.io
- package.json pnpm config clean (no contradictory directives)
- Contact form migrated from Netlify Forms to Formspree with placeholder form ID
