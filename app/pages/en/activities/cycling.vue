<script setup lang="ts">
const siteUrl = 'https://www.pension-volgenandt.de'

definePageMeta({
  breadcrumb: { label: 'Cycling' },
})

// Fetch activity data
const { data: activity } = await useAsyncData('en-activity-radfahren', () =>
  queryCollection('activitiesEn').where('slug', '=', 'radfahren').first(),
)

if (!activity.value) {
  throw createError({ statusCode: 404, message: 'Activity not found' })
}

// SEO from YAML
useSeoMeta({
  title: activity.value.seoTitle,
  ogTitle: activity.value.seoTitle,
  description: activity.value.seoDescription,
  ogDescription: activity.value.seoDescription,
  ogImage: `${siteUrl}${activity.value.heroImage}`,
  ogType: 'website',
})

useHead({
  htmlAttrs: { lang: 'en' },
  link: [
    { rel: 'canonical', href: `${siteUrl}/en/activities/cycling/` },
    {
      rel: 'alternate',
      hreflang: 'de',
      href: `${siteUrl}/aktivitaeten/radfahren/`,
    },
    {
      rel: 'alternate',
      hreflang: 'en',
      href: `${siteUrl}/en/activities/cycling/`,
    },
    {
      rel: 'alternate',
      hreflang: 'x-default',
      href: `${siteUrl}/aktivitaeten/radfahren/`,
    },
  ],
})

const difficultyBadge: Record<string, { label: string; class: string }> = {
  leicht: { label: 'Easy', class: 'bg-sage-100 text-sage-700' },
  mittel: { label: 'Moderate', class: 'bg-waldhonig-100 text-waldhonig-700' },
  schwer: { label: 'Difficult', class: 'bg-charcoal-100 text-charcoal-700' },
}
</script>

<template>
  <div v-if="activity">
    <!-- Banner -->
    <SharedPageBanner
      :image="activity.heroImage"
      :image-alt="activity.heroImageAlt"
      title="Cycling in the Eichsfeld"
      subtitle="The Leine Cycle Path passes right by"
    />

    <!-- Intro -->
    <section class="mx-auto max-w-3xl px-6 py-12 md:py-16">
      <p class="text-lg leading-relaxed text-sage-800">
        {{ activity.intro }}
      </p>
    </section>

    <!-- Region description -->
    <section class="bg-sage-50 px-6 py-12 md:py-16">
      <div class="mx-auto max-w-3xl">
        <h2 class="mb-6 font-serif text-2xl font-semibold text-sage-900">
          The Eichsfeld Cycling Region
        </h2>
        <p class="leading-relaxed text-sage-700">
          {{ activity.regionDescription }}
        </p>
      </div>
    </section>

    <!-- Route recommendations -->
    <section v-if="activity.routes && activity.routes.length > 0" class="px-6 py-12 md:py-16">
      <div class="mx-auto max-w-4xl">
        <h2 class="mb-8 font-serif text-2xl font-semibold text-sage-900">
          Recommended Cycling Tours
        </h2>
        <div class="space-y-6">
          <div
            v-for="route in activity.routes"
            :key="route.name"
            class="rounded-lg border border-sage-200 bg-white p-6 shadow-sm"
          >
            <div class="flex flex-wrap items-start justify-between gap-4">
              <div class="flex-1">
                <h3 class="flex items-center gap-2 font-serif text-lg font-semibold text-sage-900">
                  <Icon name="ph:bicycle-duotone" class="size-6 text-sage-600" />
                  {{ route.name }}
                </h3>
                <p class="mt-2 text-sm leading-relaxed text-sage-700">
                  {{ route.highlight }}
                </p>
              </div>
              <div class="flex items-center gap-3">
                <span class="text-sm font-medium text-sage-600">
                  {{ route.distance }}
                </span>
                <span
                  :class="difficultyBadge[route.difficulty]?.class"
                  class="rounded-full px-3 py-1 text-xs font-semibold"
                >
                  {{ difficultyBadge[route.difficulty]?.label }}
                </span>
              </div>
            </div>
            <a
              v-if="route.externalUrl"
              :href="route.externalUrl"
              target="_blank"
              rel="noopener noreferrer"
              class="mt-4 inline-flex items-center gap-1 text-sm font-medium text-waldhonig-600 hover:text-waldhonig-700"
            >
              View route
              <Icon name="ph:arrow-square-out" class="size-4" />
            </a>
          </div>
        </div>
      </div>
    </section>

    <!-- External portals -->
    <section
      v-if="activity.externalPortals && activity.externalPortals.length > 0"
      class="bg-cream px-6 py-12 md:py-16"
    >
      <div class="mx-auto max-w-3xl text-center">
        <h2 class="mb-6 font-serif text-2xl font-semibold text-sage-900">Discover More Tours</h2>
        <p class="mb-8 text-sage-700">Find more cycling tours in the Eichsfeld on these portals:</p>
        <div class="flex flex-wrap justify-center gap-4">
          <a
            v-for="portal in activity.externalPortals"
            :key="portal.name"
            :href="portal.url"
            target="_blank"
            rel="noopener noreferrer"
            class="inline-flex items-center gap-2 rounded-lg border border-sage-200 bg-white px-6 py-3 font-medium text-sage-800 transition-colors hover:border-sage-400 hover:shadow-sm"
          >
            {{ portal.name }}
            <Icon name="ph:arrow-square-out" class="size-4 text-sage-500" />
          </a>
        </div>
      </div>
    </section>

    <!-- Soft CTA -->
    <SharedSoftCta text="Questions about cycling? We know the best routes." />

    <!-- Booking CTA -->
    <SharedBookingCta text="The ideal base for your cycling tour" button-text="View rooms" />
  </div>
</template>
